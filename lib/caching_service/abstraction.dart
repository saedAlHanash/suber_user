...
abstract class MCubit<AbstractState> extends Cubit<AbstractState> {
  MCubit(AbstractState initialState) : super(initialState);
  ...
  Future<Iterable<dynamic>?> addOrUpdateDate(List<dynamic> data, {
    String? name,
    String? cachedFilter,
  }) async {
  ...
  Future<void> getDataAbstract<T>({
    ...
    if (checkData) {
      loggerObject.v('$nameCache stopped on cache');
      return;
    }
